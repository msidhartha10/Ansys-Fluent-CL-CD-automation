/* moment_aoa_udf.c
   - Computes lift, drag, moments, and coefficients at AoA from aoa.txt
   - Ensures results file has only one header and one row per unique AoA
*/
...
static int header_written = 0;     /* global so it persists across calls */
static real last_written_aoa = -9999.0; /* track last AoA written */
...
DEFINE_ON_DEMAND(compute_forces_and_write)
{
    ...
    /* Prevent duplicate rows: only write if AoA is new */
    if (fabs(aoa_deg - last_written_aoa) < 1e-6)
    {
        Message("aoa_udf: AoA %g already written, skipping...\n", aoa_deg);
        return;
    }
    ...
    /* === Step 9: remember last AoA written === */
    last_written_aoa = aoa_deg;

    Message("aoa_udf: AoA %g written: Cl=%g Cd=%g Cm=%g\n",
            aoa_deg, Cl, Cd, Cmy);
}
